...
export const getCarServiceList = ({ ...params }: ICarServiceSearch) => {
  return flyio.get<ICarServiceData>("admin/carService", params).then(data => data.data);
};

// 获取增值服务详情详情
export const getCarServiceDetail = (id: number) => {
  return flyio.get<ICarService>(`admin/carService/${id}`).then(data => data.data);
};

// 删除增值服务
export const deleteCarService = (id: number) => {
  return flyio.delete(`admin/carService/${id}`).then(data => data.data);
};

// 创建增值服务
export const postCarService = (params: ICarService) => {
  return flyio.post("admin/carService", params).then(data => data.data);
};

// 编辑增值服务
export const putCarService = ({ id, ...params }: ICarService) => {
  return flyio.put(`admin/carService/${id}`, params).then(data => data.data);
};
