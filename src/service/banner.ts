...
export function getBannerDetail(id: any) {
  return fly.get<IBananer>(`banners/${id}`).then(data => data.data);
}

// 删除banner
export const deleteBanner = (id: number) => {
  return fly.delete(`banners/${id}`).then(data => data.data);
};

// 创建banner
export const postBanner = (params: IBananer) => {
  return fly.post("banners", params).then(data => data.data);
};

// 编辑banner
export const putBanner = ({ id, ...params }: IBananer) => {
  return fly.put(`banners/${id}`, params).then(data => data.data);
};
