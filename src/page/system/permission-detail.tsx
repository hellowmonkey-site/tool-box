  ...
  setup: (props, ctx) => {
    ...
    const handleSubmit = (params: IPermission) => {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          return (isAddPage ? postPermission({ ...params, parent_id: Number(route.query.parent_id || 0) }) : putPermission(form)).then(e => {
            router.back();
      ...
      <Form model={form} labelCol={{ sm: 4 }} onFinish={e => handleSubmit(e)}>
        ...
        <FormItem name="app_id" label="APP" rules={[{ required: true, message: "请先输入app_id" }]}>
           <Select v-model={[form.app_id, "value"]}>
            {
              ()=> (
                appDataSource.value.map(item => (<SelectOption value={item.id}>
                  {item.name}
                </SelectOption>))
              )
            }
        ...
        <FormItem name="module_id" label="所属模块" rules={[{ required: true, message: "请选择所属模块" }]}>
          <Select v-model={[form.module_id, "value"]}>
            {
              ()=> (
                moduleDataSource.value.map(item => (<SelectOption value={item.id}>
                  {item.title}
                </SelectOption>))
              )
            }
