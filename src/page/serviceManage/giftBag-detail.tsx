...
export default defineComponent({
  ...
  setup: (props, ctx) => {
    ...
    const firstCouponId = ref<string>("") //第一个增值券id   必选
            ...
            >
              <FormItem name="coupon" label="增值券添加" rules={[{required: true, message: "请选择增值券"}]} >
                <div class="d-flex align-items-start justify-between">
                  <Select 
                    placeholder="请选择增值券" 
                    class="d-flex flex-item-extend"> {
                    couponList.value.map(item=>{
                        return <SelectOption value={item.id}>
                                {item.name}
                              </SelectOption>
                        })}
                  </Select>
                  ...
                  {form.coupon.length===0?<PlusCircleOutlined style={{"font-size": "20px", "margin-top": "10px"}} onClick={clickPlusCoupon}/>:null}
                </FormItem>
            </Col>
            {form.coupon.length>0?
              form.coupon.map((currentCoupon, index)=>{
                return <Col span={20} push={4}>
                  <FormItem>
                    <div class={["d-flex justify-start direction-row align-items-center", index>0?"mar-t-3":""]}>
                    <Select placeholder="请选择增值券" 
                            class="d-flex flex-item-extend"> {
                          couponList.value.map(item=>{
                          return <SelectOption value={item.id}>
                                  {item.name}
                      ...
                      <MinusCircleOutlined style={{"font-size": "20px", "margin-left": "10px"}} onClick={()=>{clickMinusCoupon(index)}}/>
                    </div>
                    {index===form.coupon.length-1?<PlusCircleOutlined style={{"font-size": "20px", "margin-top": "10px"}} onClick={clickPlusCoupon}/>:null}
                  </FormItem>
                </Col>
              }):null}
          </Row>
        <FormItem hasFeedback={true} class="d-flex flex-item-extend" name="price" label="报价">
          <InputNumber class="d-flex full-width" placeholder="请输入报价" precision={2} min={0} v-model={[form.price, "value"]}></InputNumber>
        </FormItem>
        <FormItem label="限制金额" rules={[{required: true}]}>
          <div class="d-flex justify-around direction-row align-items-center">
            <FormItem name="start_money" class="d-flex flex-item-extend">
              <InputNumber class="full-width" precision={2} min={0} placeholder="请输入限制金额" v-model={[form.start_money, "value"]}></InputNumber>
            </FormItem>
            <FormItem>
              <div class="mar-l-5 mar-r-5"> - </div>
            </FormItem>
            <FormItem label="" class="d-flex flex-item-extend">
              <InputNumber name="end_money" class="full-width" precision={2} min={0} placeholder="请输入限制金额" v-model={[form.end_money, "value"]}></InputNumber>
            </FormItem>
          </div>
        </FormItem>
