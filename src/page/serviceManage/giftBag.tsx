        ...
        customRender({ record }: TableData) {
          return (
            <>
              <List 
                size="small"
                dataSource={(record.coupon.map((item: ICoupon)=>(item.name)))}
              ></List>
            </>
          );
        },
    ...
    return () => (
      <div>
        ...
        <Card 
          ...
          title={(<span class="font-normal font-small">共<span class="font-primary font-bold font-large">{dataSource.totals}</span>个增值礼包</span>)}
          extra={(<div class='d-flex justify-center direction-row align-items-center'><RouterLink to={{ name: "service-giftBag-add" }} class="ant-btn ant-btn-primary">
                  设置申请
                  </RouterLink></div>)}
          >
